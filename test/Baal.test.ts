...
describe('Baal contract', function () {
  ...
  beforeEach(async function () {
    ...
    const configurePeriods = baal.interface.encodeFunctionData('setPeriods', [
      abiCoder.encode(
    ...
    const configureGuildToken = baal.interface.encodeFunctionData('setGuildTokens', [[weth.address]])
    const configureShaman = baal.interface.encodeFunctionData('setShamans', [[shaman.address], true])
    const configureShares = baal.interface.encodeFunctionData('mintShares', [[summoner.address], [shares]])
    const configureLoot = baal.interface.encodeFunctionData('mintLoot', [[summoner.address], [loot]])

    const multi_action = encodeMultiAction(
      multisend,
      [configurePeriods, configureGuildToken, configureShaman, configureShares, configureLoot],
      [baal.address, baal.address, baal.address, baal.address, baal.address],
      [0, 0, 0, 0, 0]
    )

    const baalInitializer = abiCoder.encode(
      ['string', 'string', 'address', 'bytes'],
      [deploymentConfig.TOKEN_NAME, deploymentConfig.TOKEN_SYMBOL, multisend.address, multi_action]
    )

    await baal.setUp(baalInitializer)
