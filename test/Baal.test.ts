...
describe('Baal contract', function () {
  ...
  describe.only('shaman actions', function () {
    ...
    it('happy case - allows a proposal to enable a shaman', async function () {
      ...
      const enableShamanAction = await baal.interface.encodeFunctionData('setShamans', [[summoner.address], [1]])

      console.log("submitting proposal")
      await submitAndProcessProposal(baal, enableShamanAction)
      console.log("proposal processed")

      expect(await baal.shamans(summoner.address)).to.be.equal(1)
    })

    it.only('happy case - allows a shaman to mint shares', async function () {
      const minting = 100
      expect(await baal.balanceOf(summoner.address)).to.equal(shares)
      console.log("minting shares!! s2: %s, summoner %s, baal %s", s2.address, summoner.address, baal.address)
      await expect(
