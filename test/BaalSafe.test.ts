...
const getBaalParams = async function (
    ...
    const initalizationActions = [setAdminConfig, setGovernanceConfig, setShaman, mintLoot, mintShares, posterFromBaal]
...
describe('Baal contract', function () {
    ...
    describe('constructor', function () {
        it('verify deployment parameters', async function () {
            ...
            const summonerSelfDelegates = await sharesToken.delegates(summoner.address)
    ...
    describe('shaman actions - permission level 7 (full)', function () {
        ...
        it('mint shares - require fail - array parity', async function () {
            await expect(shamanBaal.mintShares([summoner.address], [69, 69])).to.be.revertedWith(revertMessages.mintSharesArrayParity)
        })
        ...
        it('burn shares - require fail - array parity', async function () {
            await expect(shamanBaal.burnShares([summoner.address], [69, 69])).to.be.revertedWith(revertMessages.burnSharesArrayParity)
        })

        it('burn shares - require fail - insufficent shares', async function () {
            await expect(shamanBaal.burnShares([summoner.address], [101])).to.be.revertedWith(revertMessages.burnSharesInsufficientShares)
        })
        ...
        it('mint loot - require fail - array parity', async function () {
            await expect(shamanBaal.mintLoot([summoner.address], [69, 69])).to.be.revertedWith(revertMessages.mintSharesArrayParity)
        })
        ...
        it('burn loot - require fail - array parity', async function () {
            await expect(shamanBaal.burnLoot([summoner.address], [69, 69])).to.be.revertedWith(revertMessages.burnLootArrayParity)
        })

        it('burn loot - require fail - insufficent shares', async function () {
            await expect(shamanBaal.burnLoot([summoner.address], [501])).to.be.revertedWith(revertMessages.burnLootInsufficientShares)
        })
        ...
        it('assert events emitted for minting shares', async function () {
            ...
            await expect(shamanBaal.mintShares([summoner.address], [minting]))
                .to.emit(baal, 'TransferShares')
                .withArgs(zeroAddress, summoner.address, minting)
                .to.emit(sharesToken, 'DelegateVotesChanged')
                .withArgs(summoner.address, shares, shares + minting)
            expect(await sharesToken.balanceOf(summoner.address)).to.equal(shares + minting)
        })

        it('assert events emitted for burning shares', async function () {
            const burning = 100
            expect(await sharesToken.balanceOf(summoner.address)).to.equal(shares)
            await expect(shamanBaal.burnShares([summoner.address], [burning]))
                .to.emit(baal, 'TransferShares')
                .withArgs(summoner.address, zeroAddress, burning)
                .to.emit(sharesToken, 'DelegateVotesChanged')
                .withArgs(summoner.address, shares, shares - burning)
            expect(await sharesToken.balanceOf(summoner.address)).to.equal(shares - burning)
        })

        it('assert events emitted for minting loot', async function () {
            const minting = 100
            expect(await lootToken.balanceOf(summoner.address)).to.equal(loot)
            await expect(shamanBaal.mintLoot([summoner.address], [minting]))
                .to.emit(baal, 'TransferLoot')
                .withArgs(zeroAddress, summoner.address, minting)
        ...
        it('assert events emitted for burning loot', async function () {
            const burning = 100
            expect(await lootToken.balanceOf(summoner.address)).to.equal(loot)
            await expect(shamanBaal.burnLoot([summoner.address], [burning]))
                .to.emit(baal, 'TransferLoot')
                .withArgs(summoner.address, zeroAddress, burning)
        ...
        it('have shaman mint and burn _delegated_ shares', async function () {
            ...
            expect(await sharesToken.delegates(applicant.address)).to.equal(applicant.address)
            ...
            expect(await sharesToken.balanceOf(applicant.address)).to.equal(minting)
            expect(await sharesToken.delegates(applicant.address)).to.equal(summoner.address)
            ...
            expect(await sharesToken.balanceOf(applicant.address)).to.equal(2 * minting)
            expect(await sharesToken.delegates(applicant.address)).to.equal(summoner.address)
            expect(await baal.getCurrentVotes(applicant.address)).to.equal(0)
        ...
        it('setGovernanceConfig - doesnt set voting/grace if =0', async function () {
            const governanceConfig = abiCoder.encode(['uint32', 'uint32', 'uint256', 'uint256', 'uint256', 'uint256'], [0, 0, 50, 1, 2, 3])
        ...
        it('cancelProposal - require fail - !voting (submitted)', async function () {
            ...
            await expect(baal.cancelProposal(1)).to.be.revertedWith(revertMessages.cancelProposalNotVoting)
        })
    ...
    describe('shaman permissions: 0-6', function () {
        ...
        it('permission = 0 - all actions fail', async function () {
            // admin
            await expect(shamanBaal.setAdminConfig(true, true)).to.be.revertedWith(revertMessages.baalOrAdmin)

            // manager
            await expect(shamanBaal.mintShares([shaman.address], [69])).to.be.revertedWith(revertMessages.baalOrManager)
            await expect(shamanBaal.burnShares([shaman.address], [69])).to.be.revertedWith(revertMessages.baalOrManager)
            await expect(shamanBaal.mintLoot([shaman.address], [69])).to.be.revertedWith(revertMessages.baalOrManager)
            await expect(shamanBaal.burnLoot([shaman.address], [69])).to.be.revertedWith(revertMessages.baalOrManager)

            // governor
            await expect(shamanBaal.setGovernanceConfig(governanceConfig)).to.be.revertedWith(revertMessages.baalOrGovernor)

            await baal.submitProposal(proposal.data, proposal.expiration, proposal.baalGas, ethers.utils.id(proposal.details))
            await expect(shamanBaal.cancelProposal(2)).to.be.revertedWith(revertMessages.cancelProposalNotCancellable)
        })
        ...
        it('permission = 4 - governor actions succeed', async function () {
            // admin - fail
            await expect(s4Baal.setAdminConfig(true, true)).to.be.revertedWith(revertMessages.baalOrAdmin)

            // manager - fail
            await expect(s4Baal.mintShares([s4.address], [69])).to.be.revertedWith(revertMessages.baalOrManager)
            await expect(s4Baal.burnShares([s4.address], [69])).to.be.revertedWith(revertMessages.baalOrManager)
            await expect(s4Baal.mintLoot([s4.address], [69])).to.be.revertedWith(revertMessages.baalOrManager)
            await expect(s4Baal.burnLoot([s4.address], [69])).to.be.revertedWith(revertMessages.baalOrManager)
    ...
    describe('erc20 shares - approve', function () {
        it('happy case', async function () {
            ...
            const allowance = await sharesToken.allowance(summoner.address, shaman.address)
        ...
        it('overwrites previous value', async function () {
            ...
            const allowance2 = await sharesToken.allowance(summoner.address, shaman.address)
    ...
    describe('erc20 shares - transfer', function () {
        it('transfer to first time recipient - auto self delegates', async function () {
            ...
            await summonerSharesToken.transfer(shaman.address, deploymentConfig.SPONSOR_THRESHOLD)
            ...
            const summonerCheckpoints = await sharesToken.numCheckpoints(summoner.address)
            const shamanCheckpoints = await sharesToken.numCheckpoints(shaman.address)
        ...
        it('require fails - shares paused', async function () {
            ...
            await expect(sharesToken.transfer(shaman.address, deploymentConfig.SPONSOR_THRESHOLD)).to.be.revertedWith(
                revertMessages.sharesTransferPaused
            )
        })

        it('require fails - insufficient balance', async function () {
            await expect(sharesToken.transfer(shaman.address, 101)).to.be.revertedWith(revertMessages.sharesInsufficientBalance)
        })
        ...
        it('transferring to shareholder w/ delegate assigns votes to delegate', async function () {
            ...
            const summonerCheckpoints = await sharesToken.numCheckpoints(summoner.address)
            const shamanCheckpoints = await sharesToken.numCheckpoints(shaman.address)
            const applicantCheckpoints = await sharesToken.numCheckpoints(applicant.address)
            const summonerCP0 = await sharesToken.checkpoints(summoner.address, 0)
            const summonerCP1 = await sharesToken.checkpoints(summoner.address, 1)
    ...
    describe('erc20 shares - transferFrom', function () {
        ...
        it('require fails - shares paused', async function () {
            await shamanBaal.setAdminConfig(true, false) // pause shares
            await sharesToken.approve(shaman.address, deploymentConfig.SPONSOR_THRESHOLD)
            await expect(sharesToken.transferFrom(summoner.address, shaman.address, deploymentConfig.SPONSOR_THRESHOLD)).to.be.revertedWith(
                revertMessages.sharesTransferPaused
            )
        })
        ...
        it('require fails - insufficeint approval', async function () {
            ...
            await expect(sharesToken.transferFrom(summoner.address, shaman.address, 2)).to.be.revertedWith(revertMessages.sharesInsufficientApproval)
        })
    })
    ...
    describe('erc20 loot - transfer', function () {
        ...
        it('require fails - loot paused', async function () {
            ...
            await expect(lootToken.transfer(shaman.address, 1)).to.be.revertedWith(revertMessages.lootTransferPaused)
        })

        it('require fails - insufficient balance', async function () {
            await expect(lootToken.transfer(shaman.address, 501)).to.be.revertedWith(revertMessages.lootInsufficientBalance)
        })
    })
    ...
    describe('erc20 loot - transferFrom', function () {
        ...
        it('require fails - loot paused', async function () {
            await shamanBaal.setAdminConfig(false, true) // pause loot
            await lootToken.approve(shaman.address, 500)
            await expect(shamanLootToken.transferFrom(summoner.address, shaman.address, 500)).to.be.revertedWith(revertMessages.lootTransferPaused)
        })

        it('require fails - insufficient balance', async function () {
            await lootToken.approve(shaman.address, 500)
            await expect(shamanLootToken.transferFrom(summoner.address, shaman.address, 501)).to.be.revertedWith(
        ...
        it('require fails - insufficeint approval', async function () {
            ...
            await expect(shamanLootToken.transferFrom(summoner.address, shaman.address, 500)).to.be.revertedWith(
                revertMessages.lootInsufficientApproval
            )
        })
    })
    ...
    describe('submitProposal', function () {
        ...
        it('require fail - expiration passed', async function () {
            ...
            await expect(baal.submitProposal(proposal.data, now, proposal.baalGas, ethers.utils.id(proposal.details))).to.be.revertedWith(
                revertMessages.submitProposalExpired
            )
        })
    ...
    describe('sponsorProposal', function () {
        ...
        it('require fail - proposal expired', async function () {
            ...
            const expiration = now + deploymentConfig.VOTING_PERIOD_IN_SECONDS + deploymentConfig.GRACE_PERIOD_IN_SECONDS + 1000

            await shamanBaal.submitProposal(proposal.data, expiration, proposal.baalGas, ethers.utils.id(proposal.details))
            await moveForwardPeriods(2)
            console.log(now > expiration, proposal.baalGas)

            // TODO: fix
            await expect(baal.sponsorProposal(1)).to.be.revertedWith(revertMessages.sponsorProposalExpired)
        })

        it('edge case - expiration exists, but far enough ahead 2', async function () {
            const now = await blockTime()
            const expiration = now + deploymentConfig.VOTING_PERIOD_IN_SECONDS + deploymentConfig.GRACE_PERIOD_IN_SECONDS + 100000
            await baal.submitProposal(proposal.data, expiration, proposal.baalGas, ethers.utils.id(proposal.details))
        ...
        it('require fail - not sponsor', async function () {
            ...
            await expect(shamanBaal.sponsorProposal(1)).to.be.revertedWith(revertMessages.sponsorProposalSponsor)
        })
        ...
        it('require fail - proposal doesnt exist', async function () {
            ...
            await expect(baal.sponsorProposal(1)).to.be.revertedWith(revertMessages.sponsorProposalNotSubmitted)
        })
    ...
    describe('submitVote (w/ auto self-sponsor)', function () {
        ...
        it('happy case - yes vote', async function () {
            ...
            const votes = (await sharesToken.checkpoints(summoner.address, nCheckpoints.sub(1))).votes
        ...
        it('require fail - voting period has ended', async function () {
            ...
            const state = await baal.state(1)
            expect(state).to.equal(STATES.DEEFEATED)
            await expect(baal.submitVote(1, no)).to.be.revertedWith(revertMessages.submitVoteNotVoting)
        })

        it('require fail - already voted', async function () {
            await baal.submitVote(1, yes)
            await expect(baal.submitVote(1, yes)).to.be.revertedWith(revertMessages.submitVoteVoted)
        })

        it('require fail - not a member', async function () {
            await expect(shamanBaal.submitVote(1, yes)).to.be.revertedWith(revertMessages.submitVoteMember)
        })
    ...
    describe('processProposal', function () {
        ...
        it('has enough baalGas', async function () {
            ...
            await baal.processProposal(1, proposal.data, { gasPrice: ethers.utils.parseUnits('100', 'gwei'), gasLimit: 10000000 })
        ...
        it('require fail - proposal does not exist', async function () {
            ...
            const state = await baal.state(2)
            expect(state).to.equal(STATES.UNBORN)
            await expect(baal.processProposal(2, proposal.data)).to.be.revertedWith(revertMessages.processProposalNotReady)
        })
        ...
        it('require fail - prev proposal not processed', async function () {
            ...
            await baal.submitVote(2, yes)
            await moveForwardPeriods(2)
            await expect(baal.processProposal(2, proposal.data)).to.be.revertedWith('prev!processed')
        })
        ...
        it('require fail - proposal data mismatch on processing', async function () {
            ...
            await baal.submitVote(1, yes)
            await moveForwardPeriods(2)
            await expect(baal.processProposal(1, badSelfTransferAction)).to.be.revertedWith('incorrect calldata')
        })

        it('require fail - proposal not in voting', async function () {
            await shamanBaal.submitProposal(proposal.data, proposal.expiration, proposal.baalGas, ethers.utils.id(proposal.details))
            await expect(baal.processProposal(1, proposal.data)).to.be.revertedWith(revertMessages.processProposalNotReady) // fail at submitted
            await baal.sponsorProposal(1)
            await expect(baal.processProposal(1, proposal.data)).to.be.revertedWith(revertMessages.processProposalNotReady) // fail at voting
            await baal.submitVote(1, yes)
            const beforeProcessed = await baal.proposals(1)
            await moveForwardPeriods(1)
            const state1 = await baal.state(1)
            expect(state1).to.equal(STATES.GRACE)
            await expect(baal.processProposal(1, proposal.data)).to.be.revertedWith(revertMessages.processProposalNotReady) // fail at grace
            await moveForwardPeriods(1)
        ...
        it('require fail - proposal expired 2', async function () {
            const now = await blockTime()
            const expiration = now + deploymentConfig.VOTING_PERIOD_IN_SECONDS + deploymentConfig.GRACE_PERIOD_IN_SECONDS + 2
        ...
        it('scenario - offer tribute unsafe', async function () {
            ...
            const offerWeth = weth.interface.encodeFunctionData('transferFrom', [applicant.address, gnosisSafe.address, 100])
        ...
        it('happy case - mint shares via proposal', async function () {
            ...
            await expect(submitAndProcessProposal(baal, mintSharesAction, 1)).to.emit(baal, 'ProcessProposal').withArgs(1, true, false)
        ...
        it('happy case - burn shares via proposal', async function () {
            ...
            await expect(submitAndProcessProposal(baal, burnSharesAction, 1)).to.emit(baal, 'ProcessProposal').withArgs(1, true, false)
        ...
        it('happy case - mint loot via proposal', async function () {
            ...
            await expect(submitAndProcessProposal(baal, mintLootAction, 1)).to.emit(baal, 'ProcessProposal').withArgs(1, true, false)
        ...
        it('happy case - burn loot via proposal', async function () {
            ...
            await expect(submitAndProcessProposal(baal, burnLootAction, 1)).to.emit(baal, 'ProcessProposal').withArgs(1, true, false)
    ...
    describe('ragequit', function () {
        ...
        it('require fail - enforces ascending order', async function () {
            ...
            const tokens = [lootToken.address, weth.address]
                ...
                .reverse()
            await expect(baal.ragequit(summoner.address, shares, loot - 300, tokens)).to.be.revertedWith(revertMessages.ragequitUnordered)
        })

        it('require fail - prevents actual duplicate', async function () {
            await weth.transfer(gnosisSafe.address, 100)
            await expect(baal.ragequit(summoner.address, shares, loot - 300, [weth.address, weth.address])).to.be.revertedWith(
    ...
    describe('getCurrentVotes', function () {
        it('happy case - account with votes', async function () {
            ...
            const nCheckpoints = await sharesToken.numCheckpoints(summoner.address)
            const checkpoints = await sharesToken.checkpoints(summoner.address, nCheckpoints.sub(1))
    ...
    describe('getPriorVotes', function () {
        ...
        it('require fail - timestamp not determined', async function () {
            const blockT = await blockTime()
            await expect(baal.getPriorVotes(summoner.address, blockT)).to.be.revertedWith('!determined')
        })
    })
})
...
describe('Baal contract - offering required', function () {
    ...
    describe('submitProposal', function () {
        ...
        it('require fail - no tribute offered', async function () {
            expect(
                shamanBaal.submitProposal(proposal.data, proposal.expiration, proposal.baalGas, ethers.utils.id(proposal.details))
